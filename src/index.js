...
const double = (numArray) => numArray.map((item)=>item*2);
...
const filterEven=(numArray) => numArray.filter((item)=>item % 2 === 0);
...
const students = (obj) => obj.filter((item)=> item.name.length > 5);
